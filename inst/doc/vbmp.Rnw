...
@  
This function predicts the target class for LOOCV.
It uses linear inner product covariance function and 24 training iterations.
Set iteration threshold to $10^{-8}$, silent mode.
...
@  
In the following cross validation code, \Robject{Kfold} is the number of folds, 
